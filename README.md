...
翻訳の主経路はWorkers AI（LLM）で、友人同士のくだけた口調（反말・タメ口）で訳すよう
プロンプトで指示している。Workers AIが失敗したときだけAzure Translatorにフォールバックする
（口調の指定はできないが、訳文は返せる）。
...
- 翻訳モデルは `worker/index.js` の `WORKERS_AI_MODEL` 定数で変更できる（現在は `@cf/qwen/qwen3.8-27b`）
